...
import (
	...
	"github.com/ibrahimsqlsql/aether/internal/optimization"
	"github.com/ibrahimsqlsql/aether/pkg/model"
)
...
func checkPathReflection(tempURL string, id int, options model.Options, rl *rateLimiter, pathReflection map[int]string) {
	...
	rl.Block(req.Host)
	resbody, _, _, vrs, err := SendReq(req, "aetherpathtest", options)
	if err != nil {
		return
	}
	if vrs {
		pointer := optimization.Abstraction(resbody, "aetherpathtest")
		smap := "Injected: "
