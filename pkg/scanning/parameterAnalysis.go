...
func processParams(target string, paramsQue chan string, results chan model.ParamResult, options model.Options, rl *rateLimiter, miningCheckerLine int, pLog *logrus.Entry) {
	...
	for k := range paramsQue {
		if optimization.CheckInspectionParam(options, k) {
			...
			tempURL, _ := optimization.MakeRequestQuery(target, k, "Aether", "PA", "toAppend", "NaN", options)
			var code string
			rl.Block(tempURL.Host)
			resbody, resp, _, vrs, err := SendReq(tempURL, "Aether", options)
			if err == nil {
			...
			_, lineSum := verification.VerifyReflectionWithLine(resbody, "Aether")
			if miningCheckerLine == lineSum {
				pLog.Debug("Hit linesum")
				pLog.Debug(lineSum)
			}
			if vrs {
				code = printing.CodeView(resbody, "Aether")
				code = code[:len(code)-5]
				pointer := optimization.Abstraction(resbody, "Aether")
				smap := "Injected: "
...
func ParameterAnalysis(target string, options model.Options, rl *rateLimiter) map[string]model.ParamResult {
	...
	if options.Mining {
		tempURL, _ := optimization.MakeRequestQuery(target, "pleasedonthaveanamelikethis_plz_plz", "Aether", "PA", "toAppend", "NaN", options)
		rl.Block(tempURL.Host)
		resBody, _, _, vrs, _ := SendReq(tempURL, "Aether", options)
		if vrs {
			_, lineSum := verification.VerifyReflectionWithLine(resBody, "Aether")
