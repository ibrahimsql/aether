...
func setPayloadVauleForBulk(payloads []string, inSeq int) ([]string, int) {
	...
	for _, payload := range payloads {
		...
		for _, r := range replacements {
			temp = strings.ReplaceAll(temp, r, "alert(AETHER_ALERT_VALUE)")
		}

		if strings.Contains(temp, "AETHER_ALERT_VALUE") {
			tmp := strings.ReplaceAll(temp, "AETHER_ALERT_VALUE", strconv.Itoa(seq))
