...
`
	...
	fmt.Println("Developed by ibrahimsql SQL")
	fmt.Println("https://github.com/ibrahimsql/aether")
	fmt.Println()
}

func printUsage() {
	fmt.Println("Usage: aether [mode] [target] [flags]")
	fmt.Println()
	fmt.Println("Modes:")
...
func scanSingleURLMode(args []string) {
	...
	// TODO: 'scanner' declared and not used. Commenting out to avoid lint error. Update after refactoring.
	// scanner := scanner.NewScanner(cfg)
	// TODO: 'result' declared and not used. Commenting out to avoid lint error. Update after refactoring.
	// var result interface{}
	// result = scanner.ScanURL(url)

	// Output results
	// outputter := output.NewOutputter(cfg)
	// TODO: Update output.NewOutputter usage after refactoring output package.
